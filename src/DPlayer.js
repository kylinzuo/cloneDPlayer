...
class DPlayer {
    ...
    constructor (option) {
        ...
        this.updateBar = (type, percentage, direction) => {
            if ((this.option.pcConfig.showBar && this.option.type === 'pc') || (this.option.h5Config.showBar && this.option.type === 'h5')) {
        ...
        this.animationFrame = () => {
            if (this.playedTime) {
                ...
                if (this.option.type === 'h5') {
                    this.option.h5Config.showBar && (this.element.getElementsByClassName('dplayer-h5-ptime')[0].innerHTML = utils.secondToTime(this.video.currentTime()))
                } else {
                    this.option.pcConfig.showBar && (this.element.getElementsByClassName('dplayer-ptime')[0].innerHTML = utils.secondToTime(this.video.currentTime()))
                }
        ...
        const thumbMove = (event) => {
            ...
            this.updateBar('played', percentage, 'width');
            if (this.option.type === 'h5') {
                this.option.h5Config.showBar && (this.element.getElementsByClassName('dplayer-h5-ptime')[0].innerHTML = utils.secondToTime(percentage * this.video.duration))
            } else {
                this.option.pcConfig.showBar && (this.element.getElementsByClassName('dplayer-ptime')[0].innerHTML = utils.secondToTime(percentage * this.video.duration))
            }
        };
        ...
        settingBox && (settingBox.innerHTML = settingHTML.original);

        const closeSetting = () => {
            if (this.option.pcConfig.showSetting) {
        ...
        mask.addEventListener('click', () => {
            this.option.pcConfig.showSetting && closeSetting();
        });
        this.option.pcConfig.showSetting && settingIcon.addEventListener('click', () => {
            openSetting();
        });
        ...
        this.option.pcConfig.showSetting && settingEvent();

        // set duration time
        if (this.video.duration !== 1) {
            // compatibility: Android browsers will output 1 at first
            if (this.option.type === 'h5') {
                this.option.h5Config.showBar && (this.element.getElementsByClassName('dplayer-h5-dtime')[0].innerHTML = this.video.duration ? utils.secondToTime(this.video.duration) : '00:00')
            } else {
                this.option.pcConfig.showBar && (this.element.getElementsByClassName('dplayer-dtime')[0].innerHTML = this.video.duration ? utils.secondToTime(this.video.duration) : '00:00')
            }
        }
        ...
        // web full screen
        this.option.type === 'pc' && this.element.getElementsByClassName('dplayer-full-in-icon')[0].addEventListener('click', () => {
        ...
        this.option.pcConfig.showContextmenu && this.element.addEventListener('contextmenu', (e) => {
            const event = e || window.event;
            event.preventDefault();
         ...
         * Switch quality
         */
        if (this.option.video.quality && this.option.pcConfig.showQuality) {
    ...
    switchVideo (video, danmaku) {
        ...
        if (danmaku) {
            ...
            if (this.option.type === 'h5') {
                this.option.h5Config.showBar && (this.element.getElementsByClassName('dplayer-h5-ptime')[0].innerHTML = '00:00')
            } else {
                this.option.pcConfig.showBar && (this.element.getElementsByClassName('dplayer-ptime')[0].innerHTML = '00:00')
            }
    ...
    refresh () {
        this.refreshButton = this.element.getElementsByClassName('dplayer-refresh-icon')[0];
        if (this.option.type === 'pc' && this.option.pcConfig.refreshButton || this.option.type === 'h5' && this.option.h5Config.showRefresh) {
    ...
    initVideo () {
        ...
        if (enablehls && Hls.isSupported()) {
            this.element.getElementsByClassName('dplayer-time').length && (this.element.getElementsByClassName('dplayer-time')[0].style.display = 'none');
            const hls = new Hls();
        ...
        this.video.on('all', 'durationchange', (i, video) => {
            if (video.duration !== 1) {           // compatibility: Android browsers will output 1 at first
                if (this.option.type === 'h5') {
                    this.option.h5Config.showBar && (this.element.getElementsByClassName('dplayer-h5-dtime')[0].innerHTML = utils.secondToTime(this.video.duration))
                } else {
                    this.option.pcConfig.showBar && (this.element.getElementsByClassName('dplayer-dtime')[0].innerHTML = utils.secondToTime(this.video.duration))
                }
            }
        });
        ...
        this.video.on('current', 'play', () => {
            this.element.querySelector('.dplayer-logo').style.display = 'none';
            if (this.paused) {
                this.play();
            }
        });
        ...
        this.video.on('current', 'pause', () => {
            this.element.querySelector('.dplayer-logo').style.display = 'block';
            if (!this.paused) {
                this.pause();
            }
        });
