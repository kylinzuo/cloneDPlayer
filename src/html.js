...
const html = {
    main: (option, index, tran) => {
        ...
        <div class="dplayer-controller" style="${option.controllBarStyle}">
            ${html.pcHtml(option, index, tran)}
        </div>
        ${option.pcConfig.showContextmenu ? html.contextmenuList(option.contextmenu) : ``}
        <div class="dplayer-notice"></div>`;
    },

    pcHtml: (option, index, tran) => {
        return `<div class="dplayer-icons dplayer-icons-left">
                <button class="dplayer-icon dplayer-play-icon">
                    ${svg('play')}
                </button>
                ${option.pcConfig.refreshButton && option.type === 'pc' ? `<button class="dplayer-icon dplayer-refresh-icon">
                    ${svg('refresh')}
                </button>` : ''}
                ${option.h5Config.showRefresh && option.type === 'h5' ? `<button class="dplayer-icon dplayer-refresh-icon">
                    ${svg('shuaxin')}
                </button>` : ''}
                ${option.h5Config.showBar ? '<span class="dplayer-time"><span class="dplayer-h5-ptime">0:00</span></span>' : ''}
                ${option.type === 'h5' ? `<div class="dplayer-volume hide">
                ...
                ${option.type === 'h5' ? '' : '<span class="dplayer-time"><span class="dplayer-ptime">0:00</span> / <span class="dplayer-dtime">0:00</span></span>'}
            </div>
            <div class="dplayer-icons dplayer-icons-right">
                ${option.h5Config.showBar ? '<span class="dplayer-time"><span class="dplayer-h5-dtime">0:00</span></span>' : ''}
                ${option.type === 'pc' && option.video.quality && option.pcConfig.showQuality ? `
                ...
                ` : ``}
                ${option.type === 'pc' && option.screenshot ? `
                <a href="#" class="dplayer-icon dplayer-camera-icon">
                    ${svg('camera')}
                </a>
                ` : ``}

                <div class="dplayer-comment ${option.type === 'pc' && option.pcConfig.showComment ? '' : 'hide'}">
                ...
                ${option.type === 'pc' && option.pcConfig.showOnLine ? `<div class="dplayer-online">
                    <button class="dplayer-icon dplayer-online-icon">
                        ${svg('eyes')}
                    </button>
                    <span class="dplayer-online-text">${option.pcConfig.onlineNumber}</span>
                </div>` : ''}
                ${option.type === 'pc' && option.pcConfig.showSetting ? `<div class="dplayer-setting">
                ...
                <div class="dplayer-full">
                    ${option.type === 'pc' ? `<button class="dplayer-icon dplayer-full-in-icon">
                        ${svg('full-in')}
                    </button>
            ...
            <div class="${option.type === 'pc' ? option.pcConfig.showBar ? 'dplayer-bar-wrap' : 'dplayer-bar-wrap hide' : option.h5Config.showBar ? 'dplayer-bar-wrap dplayer-h5bar-wrap' : 'dplayer-bar-wrap dplayer-h5bar-wrap hide'}">
